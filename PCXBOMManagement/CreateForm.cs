...
namespace CSI.PCC.PCX
{
    public partial class CreateForm : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void CreateForm_Load(object sender, EventArgs e)
        {
            ...
            #region LookUpEdit Binding - Factory
            Common.BindToLookUpEdit(cboFactory, "FACTORY", "", "PCC", false);
            //기본 설정은 세션 정보를 따름 - 수정 불가능
            cboFactory.EditValue = Common.USER_INFO.userFactory;
            #endregion

            #region LookUpEdit Binding - Season
            Common.BindToLookUpEdit(cboSeason, "SEASON", "", "SEASON", false);
            //기본 설정은 가장 상위의 아이템
            cboSeason.ItemIndex = 0;
            #endregion

            #region LookUpEdit Binding - Category
            Common.BindToLookUpEdit(cboCategory, "CATEGORY", "", "BOM_CAT", false);
            //기본 설정은 가장 상위의 아이템
            cboCategory.ItemIndex = 0;
            #endregion

            #region LookUpEdit Binding - Round
            Common.BindToLookUpEdit(cboRound, "ROUND", Common.USER_INFO.userFactory, "B", false);
            //기본 설정은 가장 상위의 아이템
            cboRound.ItemIndex = 0;
            #endregion

            #region LookUpEdit Binding - TD
            Common.BindToLookUpEdit(cboTD, "TD", "", "TD", false);
            //기본 설정은 가장 상위의 아이템
            cboTD.ItemIndex = 0;
            #endregion

            #region LookUpEdit Binding - Gender
            Common.BindToLookUpEdit(cboGender, "GENDER", "", "", false);
            //기본 설정은 가장 상위의 아이템
            cboGender.ItemIndex = 0;
            #endregion

            #region LookUpEdit Binding - Age
            Common.BindToLookUpEdit(cboAge, "AGE", "", "", false);
            //기본 설정은 가장 상위의 아이템
            cboAge.ItemIndex = 0;
            #endregion
        }
        ...
        {
            /****************************************************************
             * 업로드한 JSON BOM으로부터 데이터를 읽어와 컨트롤에 바인딩한다.
            ****************************************************************/
            ...
            if (fileOpen.ShowDialog() == DialogResult.OK)
            {
                ...
                //선택한 파일의 확장자가 json인지 확인
                if (fileExt != ".json") 
                {
                    return;
                }

                using (StreamReader file = File.OpenText(fileNames[0])) //스트림 열기
                {
                    using (JsonTextReader reader = new JsonTextReader(file))
                    {
                        JObject jObjects = (JObject)JToken.ReadFrom(reader);    //Read Text Data From JSON File Stream

                        cnvtJObjects = ReadDataFromJson(jObjects.ToString()); //JSON Obect 각각을 DataTable 형태로 변환

                        DataTable objBomHeader = cnvtJObjects.Tables[0];  //cnvtJObjects.Tables[0] : JSON의 "BOM Header" Object
                        DataTable objBomData = cnvtJObjects.Tables[1];  //cnvtJObjects.Tables[1] : JSON의 "BOM Data" Object

                        BindDataToPCXControls(objBomHeader);     //"BOM Header" Object의 값들을 Header Info. 컨트롤에 바인딩

                        BindDataToLineItemGrid(objBomData);    //"BOM Data" Object의 값들을 Line Item 그리드에 바인딩
                    }
                }
            }
        }
        ...
        private DataSet ReadDataFromJson(String _jObject)
        {
            /****************************************************************************************
            ...
            try
            {
                ...
                else
                {
                    ...
                    **********************************************************/
                    ...
                    DataRow objBomHeader = cnvtJObjects.Tables[0].Rows[0];        //JSON의 "BOM Header" 원본 값들을 가져옴
